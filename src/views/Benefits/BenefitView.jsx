...
const BenefitView = () => {
  ...
  const myCards = useMemo(() => {
    ...
    return allCards.filter((c) => 
      selectedUserCardIds.some(id => String(id) === String(c.id))
    );
      ...
      title={
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', padding: '4px 0' }}>
          <Space>
            ...
            <Text strong style={{ letterSpacing: '1px' }}>혜택 검색</Text>
          </Space>
          <Button 
            color="primary" 
            variant="outlined" 
    ...
    >
      <Space orientation="vertical" size="large" style={{ width: '100%', display: 'flex' }}>
        ...
        <div style={{ background: '#f9f9f9', padding: '16px', borderRadius: '8px' }}>
          <Text type="secondary" style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '1px', display: 'block', marginBottom: '12px' }}>내 카드 등록</Text>
          <Space.Compact style={{ width: '100%', marginBottom: '16px' }}>
            <Select
              style={{ width: '40%' }}
              placeholder="카드사"
              value={addBrandCode || undefined}
              onChange={(val) => { setAddBrandCode(val); setAddCardId(''); }}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            <Text type="secondary" style={{ fontSize: '10px', fontWeight: 900 }}>보유 카드 목록</Text>
          ...
          <List
            size="small"
            dataSource={myCards}
            locale={{ emptyText: <Text type="secondary" style={{ fontSize: '11px' }}>등록된 카드가 없습니다.</Text> }}
            renderItem={card => (
              <List.Item
                style={{ background: '#fff', marginBottom: '8px', borderRadius: '6px', border: '1px solid #f0f0f0' }}
                actions={[
                  <Button 
                    color="danger"
                    variant="text" 
              ...
              >
                <List.Item.Meta
                  ...
                  description={<Text type="secondary" style={{ fontSize: '10px' }}>{card.brandLabel}</Text>}
                />
              </List.Item>
            )}
          />
        ...
        {/* 혜택 검색 영역 */}
        <div>
          <Text type="secondary" style={{ fontSize: '10px', fontWeight: 900, letterSpacing: '1px', display: 'block', marginBottom: '12px' }}>주변 혜택 검색</Text>
          <Input
            size="large"
            placeholder="장소(마트, 공항)나 혜택을 입력하세요"
            prefix={<Search size={18} color="#bfbfbf" />}
            suffix={locationName && <AntTag color="green" icon={<Navigation size={10} style={{ marginRight: '4px' }} />}>현위치</AntTag>}
            value={searchKeyword}
            onChange={(e) => {
          ...
          <div style={{ marginTop: '20px' }}>
            {filteredBenefits.length > 0 ? (
                ...
                renderItem={(b, idx) => (
                  ...
                  >
                    <Space size="small" style={{ marginBottom: '4px' }}>
                      <Text type="secondary" style={{ fontSize: '10px', fontWeight: 'bold' }}>{b.brandLabel}</Text>
                      <Divider type="vertical" />
                      <Text type="secondary" style={{ fontSize: '10px', fontWeight: 'bold' }}>{b.cardLabel}</Text>
                    </Space>
                    <Title level={5} style={{ margin: '4px 0 8px', fontSize: '15px' }}>{b.label}</Title>
              ...
              <div style={{ textAlign: 'center', padding: '40px 0', opacity: 0.4 }}>
                ...
                <Text type="secondary" style={{ display: 'block', fontSize: '12px' }}>보유 카드를 등록하고 혜택을 검색해 보세요.</Text>
              </div>
            )}
          </div>
        </div>
